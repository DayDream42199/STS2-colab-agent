...
from cards import (make_starter_deck, make_coop_support_deck, CARD_POOL_IRONCLAD,
                    FACTORIES_BY_RARITY, TargetMode)
from enemies import (make_nibbit, make_shrinker_beetle, make_fuzzy_wurm_crawler,
...
from potions import POTION_POOL_IRONCLAD
...
def main():
    ...
    raw = ask("Number of players (1-4) [1]: ")
    num_players = int(raw) if raw in ("2", "3", "4") else 1

    ironclad = Player("Ironclad", max_hp=80, max_energy=3, deck=make_starter_deck())
    ironclad.add_relic(BURNING_BLOOD)   # real STS2 Ironclad always starts with this
    players = [ironclad]
    for i in range(2, num_players + 1):
        players.append(Player(f"Vanguard {i}", max_hp=65, max_energy=3, deck=make_coop_support_deck()))
    ...
    if mode == "2":
        ...
        if choice not in ENCOUNTERS:
            choice = "1"
        _, make_enemies = ENCOUNTERS[choice]
